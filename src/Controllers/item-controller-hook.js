...
const useItemController = () => {
    ...
    const findById = async (id, signal) => {
        return await xhrAios.get(`/api/items/find`, {
            params: {
                id
            }
        }, {signal});
    ...
    const findItemsForMonoTransaction = async (id, signal) => {
        return await xhrAios.get(`/api/items/transactions/mono`, {signal});
    }
    ...
    const restoreItem = async (id, signal) => {
        return await xhrAios.put(`/api/trash/items/restore`, null, {
            params: {
                id
            }
        }, {signal});
    ...
    const updateItem = async (item, signal) => {
        return await xhrAios.put(`/api/items/update`, item, {signal});
    }
    
    //  Change item status to false: delete mode
    const changeTract = async (id, tractId, signal) => {
        return await xhrAios.put(`/api/items/tracts/change/${id}`, null, {
            params: {
                tractId
            }
        }, {signal});
    ...
    const fetchTrashedItems = async (signal) => {
        return await xhrAios.get(`/api/trash/items`, {signal});
    }
    
    const changePkg = async (id, pkgId, signal) => {
        return await xhrAios.put(`/api/items/pkg/change/${id}`, null, {
            params: {
                pkgId
            }
        }, {signal});
