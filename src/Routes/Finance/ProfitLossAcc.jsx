...
const ProfitLossAcc = () => {
        ...
        try {
            if (data.startDate && data.endDate) {
                ...
                if(response && response.data){
                    ...
                    // indirect income
                    const indirectIncomeData = response.data.indirectIncome;
                    const indirectIncomeAmount = indirectIncomeData
                        .map(obj => obj.balance)
                        .reduce((currentVal, accumulator) => numeral(currentVal).add(accumulator).value(), 0);
                    setIndirectIncome([...indirectIncomeData]);
                    setIndirectIncomeAmount(indirectIncomeAmount);
                    const tempIndirectIncome = {
                        ledgerName: "Indirect Income",
                        drAmount: "",
                        crAmount: numeral(indirectIncomeAmount).format('₦0,0.00'),
                    }
                    arr.push(tempIndirectIncome);
                    arr.push(...indirectIncomeData);

                    boldRows.push(arr.length);

                    // indirect expenses
                    const indirectExpData = response.data.indirectExpenses;
                    setIndirectExp([...indirectExpData]);
                    const indirectExpAmount = indirectExpData
                        .map(obj => obj.balance)
                        .reduce((currentVal, accumulator) => numeral(currentVal).add(accumulator).value(), 0);
                    setIndirectExpAmount(indirectExpAmount);
                    const tempIndirectExp = {
                        ledgerName: "Indirect Expenses",
                        drAmount: "",
                        crAmount: numeral(indirectExpAmount).format('₦0,0.00'),
                    }
                    arr.push(tempIndirectExp);
                    arr.push(...indirectExpData);
                    ...
                    const nt = numeral(totalIn).subtract(totalExp).value();
                    setNettProfit(nt);

                    const nettProfit = {
                        ledgerName: "Nett Profit",
                        drAmount: "",
                        crAmount: numeral(nt).format('₦0,0.00'),
                    }
                    arr.push(nettProfit);
	...
	const reset = () => {
        ...
        setIndirectIncome([]);
        setIndirectIncomeAmount(0);
        setIndirectExp([]);
        setIndirectExpAmount(0);
        setDirectExp([]);
    ...
    const exportPDF = () => {
        ...
        const title = `Profit & Loss Account ${period}`;
        ...
        doc.setFontSize(14);
        doc.text("Profit & Loss Account", xCoordinate, 60, { align: 'center' }); // 60 is the Y-coordinate
        ...
        <div style={{minHeight: '75vh'}} className='container'>
            ...
            <div className="row p-3 mt-2">
                <div className="d-flex flex-row flex-wrap justify-content-between">
                    ...
                    <h2>{numeral(salesAccAmount).format('₦0,0.00')}</h2>
                </div>
            ...
            <div className="row p-3 mt-2">
                <div className="d-flex flex-row flex-wrap justify-content-between">
                    ...
                    <h2>{numeral(directIncomeAmount).format('₦0,0.00')}</h2>
                </div>
                <div style={{ maxHeight: "350px", overflow: 'scroll' }}>
                    <Table id="myTable" className="rounded-2" striped hover responsive>
                        <thead>
            ...
            <div className="row p-3 mt-2">
                <div className="d-flex flex-row flex-wrap justify-content-between">
                    ...
                    <h2>{numeral(costOfSalesAmount).format('₦0,0.00')}</h2>
                </div>
                <div style={{ maxHeight: "350px", overflow: 'scroll' }}>
                    <Table id="myTable" className="rounded-2" striped hover responsive>
                        <thead>
            ...
            <div className="row p-3 mt-2">
                <div className="d-flex flex-row flex-wrap justify-content-between">
                    ...
                    <h2>{numeral(directExpAmount).format('₦0,0.00')}</h2>
                </div>
                <div style={{ maxHeight: "350px", overflow: 'scroll' }}>
                    <Table id="myTable" className="rounded-2" striped hover responsive>
                        <thead>
            ...
            <div className="row p-3 mt-2">
                <div className="d-flex flex-row flex-wrap justify-content-between">
                    ...
                    <h2>{numeral(indirectIncomeAmount).format('₦0,0.00')}</h2>
                </div>
                <div style={{ maxHeight: "350px", overflow: 'scroll' }}>
                    <Table id="myTable" className="rounded-2" striped hover responsive>
                        <thead>
            ...
            <div className="row p-3 mt-2">
                <div className="d-flex flex-row flex-wrap justify-content-between">
                    ...
                    <h2>{numeral(indirectExpAmount).format('₦0,0.00')}</h2>
                </div>
                <div style={{ maxHeight: "350px", overflow: 'scroll' }}>
                    <Table id="myTable" className="rounded-2" striped hover responsive>
                        <thead>
            ...
            <div className="d-flex flex-row flex-wrap justify-content-between">
                ...
                <h2>{numeral(nettProfit).format('₦0,0.00')}</h2>
            </div>
        </div>
    )
}
